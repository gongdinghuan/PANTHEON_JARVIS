...
### 💻 现代化交互 (v4.0 Neon Glass)
- **全新 UI 设计**: 采用 "Neon Glass" 赛博朋克风格，深度玻璃拟态与霓虹点缀。
- **2-Column 布局**: 
  - **左侧**: 实时系统遥测 (CPU/RAM) 与 滚动日志 (Telemetry Stream)。
  - **右侧**: 沉浸式对话窗口，支持悬浮输入框与宽屏显示。
- **Markdown 引擎**: 
  - 完美支持 GFM (GitHub Flavored Markdown)。
  - 代码高亮、表格渲染、引用样式优化。
  - **智能链接**: 自动识别 URL 并强制新标签页打开。
- **交互优化**: 消息输入框悬浮设计，打字机效果，平滑滚动。
